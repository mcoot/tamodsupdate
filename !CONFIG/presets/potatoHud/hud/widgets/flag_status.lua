function flag_status(x, y)
	...
	if not flag.isHome(left_team) then
		...
		if return_time > 0 then
			...
			drawText(return_time, team_colors_text[left_color], x - 140, y + 22, 2, 1.2) 
		else
			-- Flag is held, display the name of the holder in the enemies color
			drawText(flag.holderName(left_team), team_colors_text[right_color], x - 140, y + 22, 2, 1) 
		end
	end
	-- Right team
	if not flag.isHome(right_team) then
		...
		if return_time > 0 then
			-- Flag is dropped, display the time until auto return
			drawText(return_time, team_colors_text[right_color], x + 140, y + 22, 0, 1.2) 
		else
			-- Flag is held, display the name of the holder in the enemies color
			drawText(flag.holderName(right_team), team_colors_text[left_color], x + 140, y + 22, 0, 1) 
		end
	end
end
